---
title: "Differential gene expresion analysis of organoids vs. other organoids"
...
```{r}
counts_df = counts_df[, !grepl("_1$|_2$|ovary|tissue", colnames(counts_df))]
```
...
pdf('../results/organoid_vs_organoid/density_raw_and_logCPM.pdf')
...
pdf('../results/organoid_vs_organoid/MDS_expr_profiles.pdf', width=9, height=7)
...
for(i in tissues){
  ...
  #openxlsx::write.xlsx(x=de_list, paste0(file='../results/organoid_vs_organoid/top_DE_genes_', i, '_vs_organoids.xlsx'))
}

#pdf('../results/organoid_vs_organoid/p_value_histograms.pdf')
...
Generate heatmaps
```{r}
fps = list.files('../results/organoid_vs_organoid/', pattern='xlsx', full.names=T)
hm_BH = list()
hm_Holm = list()
for(i in tissues){
  ...
  if(nrow(degenes_BH) > 0){
    mtx_BH = cpm(countsFilter$counts, log=T, prior.count=0.5) %>%
      ...
      #as.matrix()
    colnames(mtx_BH) = gsub('anoid', '', colnames(mtx_BH))
    hm_BH[[i]] = mtx_BH
  }
  ...
  if(nrow(degenes_Holm) > 0){
    mtx_Holm = cpm(countsFilter$counts, log=T, prior.count=0.5) %>%
      ...
      #column_to_rownames(var='gene_symbol') %>%
      #as.matrix()
    colnames(mtx_Holm) = gsub('anoid', '', colnames(mtx_Holm))
    hm_Holm[[i]] = mtx_Holm
  }
...
```{r}
pdf(paste0('../results/organoid_vs_organoid/DE_genes_organoid_vs_organoid_BHPadjust.pdf'), height=30, pointsize=8)
print(hm_BH)
dev.off()
```

```{r}
pdf(paste0('../results/organoid_vs_organoid/DE_genes_organoid_vs_organoid_HolmPadjust.pdf'))
print(hm_Holm)
dev.off()
```
...
pdf('../results/organoid_vs_organoid/organoid_DE_up_genes.pdf')
grid.draw(venn_BH)
dev.off()

v1 = Venn(org_Up)
pdf('../results/organoid_vs_organoid/organoid_DE_up_genes_6setVenn.pdf')
...
openxlsx::write.xlsx(x=vennInts, file="../results/organoid_vs_organoid/organoid_DE_up_genes_6setVenn_Intersections.xlsx")
```
...
pdf('../results/organoid_vs_organoid/MDS_expr_profiles_OnlyDEGenes.pdf', width=9, height=7)
